...
export async function runAgent({ prompt, thread, mockupHtml, settings, mcpTools, mcpClient, onEvent }) {
  ...
  if (provider === 'mock') {
    const html = mockResponse(prompt);
    emit({ type: 'text', text: '[Mock] Mockup generated.' });
    emit({ type: 'html_updated', html, explanation: '' });
    emit({ type: 'done', finalHtml: html });
    return { events, finalHtml: html };
  }
...
function mockResponse(prompt) {
  return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Mockup</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:system-ui,sans-serif;background:#fdf8ff;color:#333;display:flex;align-items:center;justify-content:center;min-height:100vh;padding:2rem}.card{background:white;border-radius:24px;padding:3rem;box-shadow:0 4px 24px rgba(0,0,0,.08);max-width:600px;text-align:center}h1{font-size:2rem;margin-bottom:1rem;color:#6c5ce7}p{line-height:1.6;color:#666;margin-bottom:1rem}.badge{display:inline-block;background:#6c5ce7;color:white;padding:.4rem 1rem;border-radius:100px;font-size:.875rem;margin-top:1rem}</style></head><body><div class="card"><h1>Mock Response</h1><p>Configure a real LLM provider to generate actual mockups.</p><span class="badge">Mock Mode</span></div></body></html>`;
}
